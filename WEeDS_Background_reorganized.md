...
$$ where $p(q)$ is the marginal probability density of the collective coordinate under the Boltzmann distribution and $k_B T$ is the thermal energy. For a system with Hamiltonian $H(x)$ over a configuration space $\Omega$, the Boltzmann distribution is $$
\pi(x) = Z^{-1} \exp\!\left(-\frac{H(x)}{k_B T}\right), \qquad Z = \int_{\Omega} \exp\!\left(-\frac{H(x)}{k_B T}\right) dx,
$$ and the marginal of $q$ is obtained by integrating out all degrees of freedom not captured by $q$: $$
p(q) = \int_{\Omega} \delta(q(x') - q)\,\pi(x')\,dx'.
$$ Substituting, $$
F(q) = -k_B T \ln \int_{\Omega} \delta(q(x') - q)\,\exp\!\left(-\frac{H(x')}{k_B T}\right) dx' + \text{const},
...
Three distributions are distinguished throughout the document and are not interchangeable. The **stationary distribution** $\pi(x)$ is the equilibrium target. The **transient distribution** $p(x, t_k)$ is the distribution of walker positions produced by WE at iteration $k$; under the Zhang–Jasnow–Zuckerman\textsuperscript{[10]} unbiasedness result, $p(x, t_k)$ is the true distribution at physical time $t_k = k\,\tau_{\mathrm{WE}}$ starting from the WE initial distribution $p(x, 0)$, and the raw WE iteration weights $w_i^{(k)}$ are unbiased estimators of this distribution in the sense $$
...
The generalized-eigenvalue-problem formulation of Klus et al.\textsuperscript{[5]} unifies the spectral estimators used throughout the pipeline: tICA, DMD, kEDMD, and VAMP all solve a single generalized eigenvalue problem $$
...
**Type 1 — Raw WE iteration weights** $w_i^{(k)}$. These are the per-walker weights produced by the WESTPA splitting-and-merging algorithm at iteration $k$, normalized so that $\sum_i w_i^{(k)} = 1$ for each iteration by WE construction. Under the Zhang–Jasnow–Zuckerman\textsuperscript{[10]} unbiasedness theorem, the raw weights provide an unbiased estimator of the transient distribution at iteration $k$: for any bounded test function $f$, $$
\mathbb{E}\left[\sum_i w_i^{(k)}\, f(x_i^{(k)})\right] = \int_{\Omega} f(x)\, p(x, t_k)\, dx.
...
WE descends from the variance-reduction ideas of von Neumann's splitting and Russian-roulette schemes for particle transport, adapted to molecular kinetics by Huber and Kim\textsuperscript{[15]}, who introduced the essential refinement of *binning*: trajectories are grouped by a progress coordinate and the splitting and merging are applied within each bin separately, so that the statistical weight carried by each bin is preserved while the number of trajectories representing that bin is equalized. The method is best understood as importance sampling in trajectory space rather than as a biased dynamics.

Each walker is an exact, unbiased dynamical trajectory; what WE controls is only how many walkers — and carrying how much probability — populate each region of configuration space. By forcing roughly equal walker counts into bins that unbiased dynamics would visit only rarely, WE allocates computational effort to the low-probability regions — barrier tops, product basins, transition channels — that dominate the statistical error of rare-event observables, while the explicit weights keep every ensemble average unbiased. The speedup over brute-force molecular dynamics comes entirely from this reallocation of sampling effort, not from any distortion of the underlying physics.

The foundational property proved by Zhang, Jasnow and Zuckerman\textsuperscript{[10]} is that the raw WE walker weights $w_i^{(k)}$ at iteration $k$ are unbiased estimators of the transient distribution $p(x, t_k)$ of the underlying unbiased dynamics at physical time $t_k = k\tau_{\mathrm{WE}}$, where $\tau_{\mathrm{WE}}$ is the WE iteration lag time. This unbiasedness holds for each iteration **individually**, regardless of whether the WE run has reached a steady state, and is the theoretical foundation on which all downstream stages of the pipeline rest. The point deserves emphasis because it is structurally different from the assumptions underlying conventional time-average MD: WE does not require ergodic convergence of any single walker, and it does not require the walker cloud as a whole to have converged to $\pi$; it requires only that the dynamics within each WE propagation segment are exact unbiased MD, and that the resampling rule conserve total weight in expectation.

The WE protocol proceeds iteratively. At iteration $k = 0$, the PC range of interest is partitioned into bins $\{B_\alpha\}_{\alpha=1}^{N_{\mathrm{bins}}}$, and an initial population of walkers is seeded across the bins with assigned weights $w_i^{(0)}$ summing to unity (or to the user-specified total probability when combining ensembles). Each walker is a physical configuration $x_i^{(k)} \in \Omega$ carrying a weight $w_i^{(k)} \in [0, 1]$. The interpretation is that the walker represents a Dirac delta at $x_i^{(k)}$ in the transient distribution with measure $w_i^{(k)}$, so that $$
...
The WE iteration count $N$ is therefore a dual-purpose parameter. In the conventional WE analysis, $N$ sets the physical horizon of the simulation; in the present pipeline, it additionally sets **the number of columns of the density-ratio matrix** $\mathbf{R} \in \mathbb{R}^{M \times N}$, which is the data matrix consumed by the DMD stage. Each WE iteration produces exactly one measurement of the evolving density $p(x, t_k)$, and the iteration count $N$ directly determines the number of snapshots available to the DMD algorithm.

Sizing $N$ correctly — specifically, ensuring $N$ exceeds the longest implied timescale of interest by several multiples and provides enough columns for the SVD rank truncation to resolve $n-1$ slow modes — is therefore a pipeline design decision that couples the WE runtime to the spectral resolution of the downstream DMD. Typical values of $N$ in this pipeline are 200–2000 iterations, with the lower end adequate for systems with a single slow timescale and two basins, and the upper end required for multi-basin systems with multiple slow timescales and fine spectral-gap resolution.

WESTPA 2.0\textsuperscript{[1]} (building on the original WESTPA release\textsuperscript{[16]}) is the reference open-source implementation of the WE algorithm used in this pipeline. The software exposes the propagation, binning, and resampling stages as modular components and supports a wide range of PC definitions, adaptive binning schemes, and parallel execution backends. The pipeline makes no algorithmic modifications to WESTPA itself; all novelty lies in the downstream analysis stages that consume the WESTPA output.

### 4.2 Resampling rules and probability conservation

The Huber-Kim splitting and merging protocol\textsuperscript{[15]} is the standard resampling rule used by WESTPA and by the pipeline. Given a target walker count $n_{\mathrm{target}}$ per occupied bin and the current set of walkers $\{(x_i, w_i)\}$ occupying a given bin after propagation, the rule proceeds as follows.
...
The formal statement \textsuperscript{[10]} is that for any bounded observable $f$ and any iteration $k$, $$
\mathbb{E}\!\left[\sum_i w_i^{(k)}\, f(x_i^{(k)})\right] \;=\; \int_\Omega f(x)\, p(x, t_k)\, dx,
...
The decomposition of the resampling step into bias-free splitting and variance-bounded merging is the key to WE's statistical behaviour. Splitting introduces no error of any kind — it is an exact refinement of the walker-delta representation — so all of the resampling variance is carried by merging, in which one configuration is discarded at each operation. The variance therefore depends on where and how often merging occurs, which is in turn governed by the bin definition and the target walker count. This observation motivates the variance-optimal allocation theory of Aristoff and Zuckerman\textsuperscript{[17]}, which characterizes the bin occupancies and splitting/merging schedule that minimize the asymptotic variance of a target observable, and the broader mathematical analysis of WE convergence, unbiasedness, and variance structure developed by Aristoff, Copperman, Simpson, Webber and Zuckerman\textsuperscript{[18]}.

More recent trajectory-management strategies further reduce variance by managing walker genealogies and merge timing more carefully\textsuperscript{[19]}. For the present pipeline the practical consequence is that bin density and target count should be chosen to keep merging — and therefore variance — low in precisely the transition regions and rare basins whose density-ratio entries carry the slow-mode signal, even though the downstream spectral estimators are otherwise constructed to be insensitive to the bin definition itself.

WE supports two distinct operating modes that differ only in the boundary condition imposed on walkers, and the pipeline's choice between them is consequential. In *steady-state* (non-equilibrium steady-state, NESS) mode, walkers that reach a designated target (sink) state are *recycled* — their weight is reinjected at a source state — so that the ensemble approaches a constant-flux stationary distribution rather than equilibrium. In this mode the mean first-passage time from source to target is recovered from the Hill relation as the reciprocal of the steady-state probability flux into the target, $\mathrm{MFPT} = 1/J_{\mathrm{SS}}$, which is the basis of WE rate-constant estimation\textsuperscript{[20]} and the dominant historical application of the method\textsuperscript{[21]}.

In *equilibrium* mode no recycling is imposed: walkers evolve under unbiased dynamics with splitting and merging only, and the iteration-indexed ensemble relaxes toward the Boltzmann distribution $\pi$ rather than toward a flux-carrying steady state\textsuperscript{[22]}. The present pipeline runs WE exclusively in the equilibrium mode (the WEED driver, described in the next subsection), because the density-ratio construction is defined relative to $\pi$ and requires walker coverage of the full support of $\pi$ on both sides of every barrier — a requirement that the NESS reactive-flux tube does not satisfy. The steady-state formalism is described here only to delineate what the pipeline deliberately does not use; the coverage-versus-crossing distinction this implies is developed below.
...
The pipeline treats this pooled distribution correctly in two complementary ways. First, any quantity that is *native* to the transient ensemble — individual $p(x, t_k)$ snapshots, time-averaged features of the walker trajectories, non-equilibrium correlations — is computed directly from the raw WE weights $w_i^{(k)}$, exploiting the Zhang-Jasnow-Zuckerman unbiasedness per iteration. The density-ratio numerator, the importance-weighted tICA covariance estimators, and the visualization KDE per iteration all fall in this category.

Second, any quantity that requires an equilibrium reference — stationary expectations, equilibrium free energies, the density-ratio denominator, kEDMD endpoint reweighting — is computed from the RiteWeight-corrected stationary weights $\hat{w}_i^\pi$, which estimate $\pi(x_i) / q_{\mathrm{agg}}(x_i)$ (the RiteWeight stationary-reweighting estimator\textsuperscript{[23]}, with a regularized variant\textsuperscript{[24]} for sparse trajectory data) and thereby convert the pooled walker set into an importance-weighted sample from $\pi$. Specifically, the identity $$
...
Stage 1 output is the **WE corpus**
$$
\mathcal{C} = \bigl\{(x_i^{(k, \mathrm{start})}, x_i^{(k, \mathrm{end})}, w_i^{(k)}): i = 1, \ldots, n_k;\; k = 1, \ldots, N\bigr\},
$$
together with auxiliary data (walker trajectories, bin assignments, parent pointers, the WESTPA HDF5 archive) that are used for diagnostics but do not enter the spectral pipeline proper. Three distinct objects are extracted downstream:
...
**Analytical content.** The spectral decomposition writes the transfer operator's eigenvalues and eigenfunctions as objects defined pointwise on $\operatorname{supp}(\pi)$, weighted by the stationary density. Equivalently, the Klus et al.\textsuperscript{[5]} generalized eigenvalue formulation recasts the spectral problem as a statement about covariance operators $C_0$ and $C_\tau$ built from samples drawn from $\pi$, where the samples need only **cover** the support — they need not trace reactive trajectories between basins.

The spectral objects (eigenvalues $\lambda_i$, eigenfunctions $\psi_i$, implied timescales $\tau_i$) are *global* properties of the transfer operator that are reconstructible from sufficient *local* samples of the operator's action, where "sufficient" means coverage of $\operatorname{supp}(\pi)$ adequate to resolve $\hat{\pi}$ and $\hat{p}(\cdot, t_k)$ at the reference set, not observation of any specific class of reactive trajectories. This is the central mathematical content of the spectral-recovery formulation and the structural reason why WE's coverage-oriented sampling is the right input to this pipeline.
...
which translates into a concrete constraint on the number of WE iterations required to recover a mode of given timescale. This is the analog of the implied-timescale plateau test of Noé & Fischer\textsuperscript{[11]}: modes whose timescales are much longer than the total simulation time are not identifiable; modes whose timescales are comparable to a single iteration are not resolved.
...
References are numbered in order of first appearance. Bibliographic details for the spectral-estimation, RiteWeight, kernel-EDMD, and validation stages that this Background document cites only in passing are given in full in the companion *Foundational Literature* guide.

1. J. D. Russo, S. Zhang, J. M. G. Leung, A. T. Bogetti, J. P. Thompson, A. J. DeGrave, P. A. Torrillo, A. J. Pratt, K. F. Wong, J. Xia, J. Copperman, J. L. Adelman, M. C. Zwier, D. N. LeBard, D. M. Zuckerman, and L. T. Chong, "WESTPA 2.0: High-Performance Upgrades for Weighted Ensemble Simulations and Analysis of Longer-Timescale Applications," *J. Chem. Theory Comput.* **18**(2), 638–649 (2022). DOI: 10.1021/acs.jctc.1c01154.

2. H. Wu, F. Nüske, F. Paul, S. Klus, P. Koltai, and F. Noé, "Variational Koopman Models: Slow Collective Variables and Molecular Kinetics from Short Off-Equilibrium Simulations," *J. Chem. Phys.* **146**(15), 154104 (2017). DOI: 10.1063/1.4979344.

3. L. Bold, F. M. Philipp, M. Schaller, and K. Worthmann, "Kernel-Based Extended Dynamic Mode Decomposition: Convergence Rates in the Operator-Norm via the Fill Distance," *SIAM J. Control Optim.* **63**(6), 4044–4071 (2025). DOI: 10.1137/24M1715945.

4. M. Hertel, F. M. Philipp, M. Schaller, and K. Worthmann, "Pointwise Error Bounds for Kernel-Based EDMD Applied to Stochastic Dynamical Systems," arXiv:2512.20247 (2026).

5. S. Klus, F. Nüske, P. Koltai, H. Wu, I. Kevrekidis, C. Schütte, and F. Noé, "Data-Driven Model Reduction and Transfer Operator Approximation," *J. Nonlinear Sci.* **28**(3), 985–1010 (2018). DOI: 10.1007/s00332-017-9437-7.

6. D. B. Rubin, "The Bayesian Bootstrap," *Ann. Statist.* **9**(1), 130–134 (1981). DOI: 10.1214/aos/1176345338.

7. B. Mostofian and D. M. Zuckerman, "Statistical Uncertainty Analysis for Small-Sample, High Log-Variance Data: Cautions for Bootstrapping and Bayesian Bootstrapping," *J. Chem. Theory Comput.* **15**(6), 3499–3509 (2019). DOI: 10.1021/acs.jctc.9b00015.

8. H. R. Künsch, "The Jackknife and the Bootstrap for General Stationary Observations," *Ann. Statist.* **17**(3), 1217–1241 (1989). DOI: 10.1214/aos/1176347265.

9. R. Y. Liu and K. Singh, "Moving Blocks Jackknife and Bootstrap Capture Weak Dependence," in *Exploring the Limits of Bootstrap* (R. LePage and L. Billard, eds.), pp. 225–248, Wiley, New York (1992).

10. B. W. Zhang, D. Jasnow, and D. M. Zuckerman, "The 'Weighted Ensemble' Path Sampling Method Is Statistically Exact for a Broad Class of Stochastic Processes and Binning Procedures," *J. Chem. Phys.* **132**(5), 054107 (2010). DOI: 10.1063/1.3306345.

11. F. Noé and S. Fischer, "Transition Networks for Modeling the Kinetics of Conformational Change in Macromolecules," *Curr. Opin. Struct. Biol.* **18**(2), 154–162 (2008). DOI: 10.1016/j.sbi.2008.01.008.

12. H. Wu and F. Noé, "Variational Approach for Learning Markov Processes from Time Series Data," *J. Nonlinear Sci.* **30**, 23–66 (2020). DOI: 10.1007/s00332-019-09567-y.

13. P. Koltai, H. Wu, F. Noé, and C. Schütte, "Optimal Data-Driven Estimation of Generalized Markov State Models for Non-Equilibrium Dynamics," *Computation* **6**(1), 22 (2018). DOI: 10.3390/computation6010022.

14. J. H. Tu, C. W. Rowley, D. M. Luchtenburg, S. L. Brunton, and J. N. Kutz, "On Dynamic Mode Decomposition: Theory and Applications," *J. Comput. Dyn.* **1**(2), 391–421 (2014). DOI: 10.3934/jcd.2014.1.391.

15. G. A. Huber and S. Kim, "Weighted-Ensemble Brownian Dynamics Simulations for Protein Association Reactions," *Biophysical Journal* **70**(1), 97–110 (1996). DOI: 10.1016/S0006-3495(96)79552-8.

16. M. C. Zwier, J. L. Adelman, J. W. Kaus, A. J. Pratt, K. F. Wong, N. B. Rego, E. Suárez, S. Lettieri, D. W. Wang, M. Grabe, D. M. Zuckerman, and L. T. Chong, "WESTPA: An Interoperable, Highly Scalable Software Package for Weighted Ensemble Simulation and Analysis," *J. Chem. Theory Comput.* **11**(2), 800–809 (2015). DOI: 10.1021/ct5010615.

17. D. Aristoff and D. M. Zuckerman, "Optimizing Weighted Ensemble Sampling of Steady States," *Multiscale Model. Simul.* **18**(2), 646–673 (2020). DOI: 10.1137/18M1212100.

18. D. Aristoff, J. Copperman, G. Simpson, R. J. Webber, and D. M. Zuckerman, "Weighted Ensemble: Recent Mathematical Developments," *J. Chem. Phys.* **158**(1), 014108 (2023). DOI: 10.1063/5.0110873.

19. W. H. Ryu, J. D. Russo, M. S. Johnson, J. T. Copperman, J. P. Thompson, D. N. LeBard, R. J. Webber, G. Simpson, D. Aristoff, and D. M. Zuckerman, "Reducing Weighted Ensemble Variance with Optimal Trajectory Management," *J. Chem. Phys.* **164**(9), 094110 (2026). DOI: 10.1063/5.0311015.

20. D. Bhatt, B. W. Zhang, and D. M. Zuckerman, "Steady-State Simulations Using Weighted Ensemble Path Sampling," *J. Chem. Phys.* **133**(1), 014110 (2010). DOI: 10.1063/1.3456985.

21. D. M. Zuckerman and L. T. Chong, "Weighted Ensemble Simulation: Review of Methodology, Applications, and Software," *Annu. Rev. Biophys.* **46**, 43–57 (2017). DOI: 10.1146/annurev-biophys-070816-033834.

22. E. Suárez, S. Lettieri, M. C. Zwier, C. A. Stringer, S. R. Subramanian, L. T. Chong, and D. M. Zuckerman, "Simultaneous Computation of Dynamical and Equilibrium Information Using a Weighted Ensemble of Trajectories," *J. Chem. Theory Comput.* **10**(7), 2658–2667 (2014). DOI: 10.1021/ct401065m.

23. S. Kania, R. J. Webber, G. Simpson, D. Aristoff, and D. M. Zuckerman, "RiteWeight: Randomized Iterative Trajectory Reweighting for Steady-State Distributions Without Discretization Error," arXiv:2401.05597v4 (2026).

24. L. Otten, D. Aristoff, and D. M. Zuckerman, "Regularized RiteWeight for Sparse Trajectory Data: A Smoothed Stationary Reweighting Framework," ChemRxiv (2026). DOI: 10.26434/chemrxiv.15001337/v1.

25. J. Copperman and D. M. Zuckerman, "Accelerated Estimation of Long-Timescale Kinetics from Weighted Ensemble Simulation via Non-Markovian 'Microbin' Analysis," *J. Chem. Theory Comput.* **16**(11), 6763–6775 (2020). DOI: 10.1021/acs.jctc.0c00273.
